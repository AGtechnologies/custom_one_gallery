...
def get_columns(filters):
	columns = [_("Document Date") + "::100",_("Document") + "::100",_("Document No.") + "::100",_("Customer's Contact Person") + "::100",_("Payment Term") + "::100",_("Days Overdue") + "::100",
		_("Amount(SGD)") + ":Currency/currency:140",
		_("0 - " + str(filters.range1) +" Days") + ":Currency/currency:100",
		_(str(filters.range1) + " - " + str(filters.range2) + " Days") + ":Currency/currency:100",
		_(str(filters.range2) + " - " + str(filters.range3) + " Days") + ":Currency/currency:100",
		_("Over " + str(filters.range3) + " Days") + ":Currency/currency:100",
		_("Total Outstanding Balance") + ":Currency/currency:100"]
...
def get_data(filters):
	...
	for cust in cust_list:
		...
		data.append([inv_date,'Invoice',cust.name,contact_person,cust.credit_days,days_overdue,cust.base_grand_total,aging1,aging2,aging3,aging4,total_bal])
...
def get_cust_list(filters):
	...
	# frappe.throw(_(aa))
	cust_list = frappe.db.sql("""select si.name, si.posting_date, si.due_date, si.naming_series, si.base_grand_total, si.contact_person, si.customer_name, cs.credit_days from `tabSales Invoice` si inner join `tabCustomer` cs where cs.name=si.customer %s order by posting_date""" %
